import { PointPerEnemyType, HI_SCORE_KEY } from '../config/constants.js';
import type { EnemyTypeValue } from '../config/constants.type.js';

export type PlayerIndex = 0 | 1;
type KillScores = Record<number, [number, number]>;

export interface Scores extends KillScores {
  total: [number, number];
  hiScore: number;
}

export class StatsManager {
  scores: Scores;

  constructor() {
    this.scores = { total: [0, 0], hiScore: 0 };

    this._resetLevelScores();

    const hiScore = localStorage.getItem(HI_SCORE_KEY);
    if (hiScore) {
      this.scores.hiScore = parseInt(hiScore, 10) || 0;
    }
  }

  reset(): void {
    this._resetLevelScores();
    this.scores.total = [0, 0];
  }

  nextLevel(): void {
    for (let enemyType = 1; enemyType <= 4; enemyType++) {
      this.scores.total[0] +=
        this.scores[enemyType][0] * PointPerEnemyType[enemyType as EnemyTypeValue];
      this.scores.total[1] +=
        this.scores[enemyType][1] * PointPerEnemyType[enemyType as EnemyTypeValue];

      this.scores[enemyType] = [0, 0];
    }

    if (this.scores.total[0] > this.scores.hiScore || this.scores.total[1] > this.scores.hiScore) {
      this.scores.hiScore = Math.max(this.scores.total[0], this.scores.total[1]);
      localStorage.setItem(HI_SCORE_KEY, this.scores.hiScore.toString());
    }
  }

  private _resetLevelScores(): void {
    for (let enemyType = 1; enemyType <= 4; enemyType++) {
      this.scores[enemyType] = [0, 0];
    }
  }

  recordKill(player: PlayerIndex, enemyType: EnemyTypeValue): void {
    this.scores[enemyType][player]++;
  }
}
